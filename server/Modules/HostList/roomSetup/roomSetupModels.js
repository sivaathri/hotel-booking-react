...
class Room {
  ...
  static async createRoom(data) {
    ...
    const facilitiesString = JSON.stringify(facilities); // <<<<<<<<<< IMPORTANT
  
    const [result] = await db.query(
      `INSERT INTO room_setup 
      (user_id, floor, room_type, number_of_rooms, capacity, bed_type, has_attached_bathroom, has_balcony, facilities) 
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        user_id,
        floor,
        ...
        has_attached_bathroom,
        has_balcony,
        facilitiesString // <<<<<<<<<< here pass the string
      ]
    );
